package io.wetabq.multiquickshop.command
...
import io.wetabq.multiquickshop.shop.ShopType
 ...
 * @version 1.0
 */
class QuickShopCommand : Command("mqs") {

    init {
        this.setDescription("MultiQuickShop Command")
        this.aliases = arrayOf("mshop", "mqshop", "mquickshop", "multishop", "multiqshop", "multiquickshop")
        this.usage = "/mqs <subcommand> [args]"
    ...
    override fun execute(sender: CommandSender, label: String, args: Array<out String>): Boolean {
        ...
        when (args[0]) {
            ...
            "version", "v" -> {
                sender.sendMessage(TextFormat.colorize(QuickShop.TITLE+"&lMulti&eQuick&6Shop &r&c- &a${QuickShop.VERSION} &dMade by WetABQ\n${QuickShop.TITLE}If you have any questions, please feel free to send us feedback to our email wetabq@gmail.com"))
            }
        }
        return true
    }
    ...
    private fun sendHelp(sender: CommandSender) {
        sender.sendMessage(Lang.getMessage("&6----MultiQuickShop Command----"))
        sender.sendMessage(Lang.getMessage("&b/mqs help(h) - View help"))
        sender.sendMessage(Lang.getMessage("&b/mqs buy(b) - Set the current clicked shop as the buy type"))
        sender.sendMessage(Lang.getMessage("&b/mqs sell(s) - Set the current clicked shop as the sell type"))
        sender.sendMessage(Lang.getMessage("&b/mqs price(p) <price: Int> - Set the current clicked shop's price"))
        sender.sendMessage(Lang.getMessage("&b/mqs unlimited(ul) - Set the current clicked shop as unlimited"))
        sender.sendMessage(Lang.getMessage("&b/mqs version(v) - View MultiQuickShop version"))
    }
}
